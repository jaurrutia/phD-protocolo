...
Multiple Scattering Theories (MSTs) provide approximate solutions to Maxwell's equations when considering the problem of light scattered by an ensemble of particles illuminated by a monochromatic plane wave \cite{loiko_monolayers_1998,reyes2018analytical}. In particular, the Coherent Scattering Model (CSM) is a MST that provides analytical expressions for the amplitude coefficients of reflection and transmission of light scattered in the coherent direction for a disordered ensemble of spherical particles with coplanar centers \cite{barrera_coherent_2003,garcia2012multiple,reyes2018analytical}. 

Within the MST framework, the optical response of a disordered array of arbitrary particles, illuminated by an incident monochromatic plane wave $\vb{E}^\text{inc}(\vb{r},\omega)$ with angular frequency $\omega$ and propagation direction $\vu{k}^\text{inc}$, is determined by the excitation field $\vb{E}^\text{exc}(\vb{r},\omega)$ acting on the $k$-th particle in a collection of $N$ particles. This field is equal to the sum of the incident electric field and the induced electric field $\vb{E}^\text{ind}_\ell(\vb{r},\omega)$ ---which includes both the field inside the particle and the scattered field outside of it--- due to the other elements of the ensemble but not itself \cite{garcia2012multiple,barrera_coherent_2003}, that is
%
\begin{align}
...
\end{align}
%
where the harmonic dependence is implied in the used notation. Since all particles are excited by the same interaction, the electric field induced in the $\ell$-th particle is given by \cite{garcia2012multiple,barrera_coherent_2003}
%
\begin{align}
...
\end{align}
%
where $\vb{r}_\ell$ is the position of the center of the $\ell$-th particle, $\mathbb{G}(\vb{r},\vb{r}')$ is the dyadic Green function ---solution to the vectorial Helmholtz equation with the product of the unitary dyadic and a Dirac delta centered at $\vb{r}'$ as a source \cite{tsang_scattering_2000}--- and $\mathbb{T}$ is the transition operator, or $T$-matrix, that linearly relates the scattered electric field of a particle to the electric field that excites it \cite{tsang_scattering_2000}. The integrals in Eq.~\eqref{eq:Eind} are performed over a  volume $V$, described by the positions $\vb{r}'$ and $\vb{r}''$, such that the center $\vb{r}_\ell$ of the $\ell$-th particle is located in $V$ \cite{barrera_coherent_2003}.

The total electric field $\vb{E}(\vb{r})$ is equal to the sum of $\vb{E}^\text{inc}(\vb{r})$ and the $N$ contributions of the induced electric field $\vb{E}^\text{ind}_\ell(\vb{r})$ for each particle, which is determined by solving the system of $N$ equations given by Ecs. \eqref{eq:Eexc} and \eqref{eq:Eind}. For an ensemble of randomly located particles, the configurational average of the total electric field $\langle\vb{E}(\vb{r})\rangle$ is calculated by considering the probability of occurrence of each combination in which the centers $\vb{r}_\ell$ of the elements can be located \cite{garcia2012multiple,barrera_coherent_2003}:
%
\begin{align}
...
\end{align}
%
with $\rho(\vb{R})$ is the probability density that the ensemble is in a specific spatial configuration given by $\vb{R} = (\vb{r}_1,\vb{r}_2,\ldots,\vb{r}_N)$. Configurationally averaging Eq.~\eqref{eq:Eind} gives the average contribution of the induced electric field by the $\ell$-th particle, which can be written as \cite{barrera_coherent_2003,garcia2012multiple}
%
\begin{subequations}%
	\begin{align}%
...
\end{subequations}%
%
where $\rho(\vb{r}_\ell)$ is the probability density of finding the center of the $\ell$-th particle in $V$, and Eq.~\eqref{eq:EexcAve} represents the configurational average of the excitation electric field $\vb{E}_\ell^\text{exc}$ for the $\ell$-th particle, considering that its position in $V$ is fixed, constriction described by the conditional probability density $\rho(\vb{R}|\vb{r}_\ell)$. This expression can be further analyzed by substituting Eq.~\eqref{eq:Eexc} into Eq.~\eqref{eq:EexcAve} and following an analogous procedure to that of Eqs. \eqref{eq:EexcSSA} with the indices exchanges $k\to\ell$ and $\ell \to m$, yielding
%
\begin{subequations}%
	\begin{align}%
