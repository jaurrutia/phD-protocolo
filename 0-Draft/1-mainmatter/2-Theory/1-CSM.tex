...
Multiple Scattering Theories (MSTs) provide approximate solutions to Maxwell's equations when considering the problem of light scattered by an ensemble of particles illuminated by a monochromatic plane wave \cite{loiko_monolayers_1998,barrera1991optical,reyes2018analytical}. In particular, the Coherent Scattering Model (CSM) is a MST that provides an expression for the amplitude coefficients of reflection and transmission of light scattered in the coherent direction for a disordered ensemble of spherical particles, whose centers are coplanar \cite{barrera_coherent_2003,garcia2012multiple,reyes2018analytical}. 
...
\end{align}
%
where the harmonic dependence is intrinsically implied. Since all particles are excited by the same interaction, the electric field induced in the $\ell$-th particle is given by \cite{garcia2012multiple,barrera_coherent_2003}
%
\begin{align}
...
\end{align}
%
where $\vb{r}_\ell$ is the position of the center of the $\ell$-th particle, $\mathbb{G}(\vb{r},\vb{r}')$ is the dyadic Green function ---solution to the vectorial Helmholtz equation with the product of the unitary dyadic and a Dirac delta centered at $\vb{r}'$ as a source \cite{tsang_scattering_2000}--- and $\mathbb{T}$ is the transition operator, or matrix $T$, that linearly relates the scattered electric field of a particle to the electric field that excites it \cite{tsang_scattering_2000}. The integrals in Eq.~\eqref{eq:Eind} are performed over a  volume $V$, described by the positions $\vb{r}'$ and $\vb{r}''$, such that the center $\vb{r}_\ell$ of the $\ell$-th particle is located within $V$ \cite{barrera_coherent_2003}.
...
\end{subequations}%
%
where $\rho(\vb{r}_\ell)$ is the probability density of finding the center of the $\ell$-th particle within the volume $V$, and Eq.~\eqref{eq:EexcAve} represents the configurational average of the excitation electric field $\vb{E}_\ell^\text{exc}$ for the $\ell$-th particle, considering that its position is fixed in $V$; the conditional probability density $\rho(\vb{R}|\vb{r}_\ell)$ describes such constriction. This expression can be further analyzed by substituting Eq.~\eqref{eq:Eexc} into Eq.~\eqref{eq:EexcAve} and following an analogous procedure to that of Eqs. \eqref{eq:EexcSSA} with the index exchanges $k\to\ell$ and $\ell \to m$, yielding
%
\begin{subequations}%
	\begin{align}%
