...
\end{align}
%
where $\vb{r}_\ell$ is the position of the center of the $\ell$-th particle, $\mathbb{G}(\vb{r},\vb{r}')$ is the dyadic Green function ---solution to the vectorial Helmholtz equation with the product of the unitary dyadic and a Dirac delta centered at $\vb{r}'$ as a source \cite{tsang_scattering_2000}--- and $\mathbb{T}$ is the transition operator, or matrix $T$, that linearly relates the scattered electric field of a particle to the electric field that excites it \cite{tsang_scattering_2000}. The integrals in Eq.~\eqref{eq:Eind} are performed over a  volume $V$, described by the positions $\vb{r}'$ and $\vb{r}''$, such that the center $\vb{r}_\ell$ of the $\ell$-th particle is located in $V$ \cite{barrera_coherent_2003}.
